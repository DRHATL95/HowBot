...
namespace Howbot.UnitTests.Core;

public class EntryPointServiceExecuteAsync
{
  private static (EntryPointService, Mock<ILoggerAdapter<EntryPointService>>, Mock<IQueueReceiver>, Mock<IServiceLocator>, Mock<IRepository>, Mock<IDiscordClientService>, Mock<Configuration>) Factory()
  {
    ...
    var discordClientService = new Mock<IDiscordClientService>();
    var configuration = new Mock<Configuration>();
    var interactionHandlerService = new Mock<IInteractionHandlerService>();
    var embedService = new Mock<EmbedService>();
    var lavaNodeService = new Mock<LavaNodeService>();
    var musicService = new Mock<MusicService>();

    // maybe a tuple later on
    var repository = SetupCreateScope(serviceLocator);

    var service = new EntryPointService(logger.Object, settings, queueReceiver.Object, null, serviceLocator.Object, null, discordClientService.Object, interactionHandlerService.Object, musicService.Object, embedService.Object, lavaNodeService.Object, configuration.Object);
    return (service, logger, queueReceiver, serviceLocator, repository, discordClientService, configuration);
  }
  ...
  {
    var (service, logger, queueReceiver, _, _, _, _) = Factory();
  ...
  public async Task MessageWasRetrievedFromTheQueue()
  {
    ...
    var (service, _, queueReceiver, _, _, _, _) = Factory();
  ...
  public async Task MessageWasRetrievedFromTheQueue_WorksManyTimes()
  {
    ...
    var (service, _, _, _, _, _,_) = Factory();
    var token = new CancellationToken(); // TODO: dhoward - this cannot be right
    await service.ExecuteAsync(token);
    await service.ExecuteAsync(token);
    await service.ExecuteAsync(token);
    await service.ExecuteAsync(token);
    var (service2, _, _, _, _, _, _) = Factory();
    await service2.ExecuteAsync(token);
    var (service3, _, _, _, _, _, _) = Factory();
    await service3.ExecuteAsync(token);
